package com.hieupham.absolutecleanarchitecturekt.data
...
fun transactionModelView1(): TransactionModelView {
    return TransactionModelView().map(transaction1())
}

fun transactionModelView2(): TransactionModelView {
    return TransactionModelView().map(transaction2())
}

fun transactionModelView3(): TransactionModelView {
    return TransactionModelView().map(transaction3())
}

fun transactionModelView4(): TransactionModelView {
    return TransactionModelView().map(transaction4())
}

fun transactionModelView5(): TransactionModelView {
...
fun compositeTransactionModelView(): List<CompositeTransactionModelView> {
    return object : ArrayList<CompositeTransactionModelView>() {
        init {
            ...
            add(CompositeTransactionModelView().map(
                    CompositeTransaction(transaction2(), asset5(), block1())))
            add(CompositeTransactionModelView().map(
                    CompositeTransaction(transaction3(), asset2(), block1())))
            add(CompositeTransactionModelView().map(
                    CompositeTransaction(transaction4(), asset4(), block1())))
            add(CompositeTransactionModelView().map(
